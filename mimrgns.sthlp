...
{phang}
{opt eform} displays (final) coefficients in exponentiated form. This 
option does not affect any calculations. 
...
{phang2}
In general, {cmd:mimrgns} passes any options thru to {cmd:margins}. Therefore, 
options affect the estimation in each imputed dataset. For most options, this 
is what you want. Some options, such as most {it:contrast_options} and option 
{opt nose} are not allowed. Also, most of {cmd:margins}' undocumented options 
are not allowed.
...
(Note: saves a file to disk!)

{phang2}
{stata "mi estimate , saving(miestfile) esample(esample) : logit attack i.smokes age bmi i.hsgrad i.female":. mi estimate , saving(miestfile) esample(esample) : logit attack i.smokes age bmi i.hsgrad i.female}
{p_end}
...
{phang2}{stata "mi estimate : logit attack i.smokes i.ageg bmi i.hsgrad i.female":. mi estimate : logit attack i.smokes i.ageg bmi i.hsgrad i.female}{p_end}
